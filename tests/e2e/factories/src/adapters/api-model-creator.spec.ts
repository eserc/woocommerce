...
describe( 'APIModelCreator', () => {
	let creator: APIModelCreator<APIModel>;
	let mockService: MockAPI;

	beforeEach( () => {
		creator = new APIModelCreator( '/wc/v3/product', () => 'test' );
	...
	it( 'should create single instance', async () => {
		...
		const result = await creator.create( mockService, new MockModel() );
	...
	it( 'should create multiple instances', async () => {
		...
		const result = await creator.create( mockService, [ new MockModel(), new MockModel(), new MockModel() ] );
